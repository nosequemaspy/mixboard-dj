...
export class AudioEngine {
  ...
  async loadSong(deckId: DeckId, songId: number, hasStems: boolean, cacheVersion: string | number = 0) {
    ...
    // cacheVersion ensures browser cache is busted when the file changes
    const streamUrl = api.streamUrl(songId) + (cacheVersion ? `?v=${cacheVersion}` : '');
