...
export function TagSidebar({
  ...
  const handleCreate = () => {
    if (newName.trim() && onCreate) {
      ...
      setNewName('');
      setNewColor(TAG_COLORS[(folders.length + 1) % TAG_COLORS.length]);
      setCreating(false);
    }
  };
  ...
  const handleDelete = (folderId: number) => {
    setContextMenu(null);
    if (onDelete && window.confirm('Eliminar etiqueta? Las canciones se quedarán en Todas.')) {
      onDelete(folderId);
  ...
  if (horizontal) {
    return (
      <div className="flex items-center gap-1 px-3 py-2 border-b border-border/40 overflow-x-auto scrollbar-none bg-bg-secondary/40">
        ...
        {folders.length > 0 && <div className="shrink-0 w-px h-4 bg-border/40 mx-0.5" />}
        {folders.map(folder => (
          <button
            key={folder.id}
          ...
          >
            <span className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: activeFolder === folder.id ? 'rgba(255,255,255,0.7)' : folder.color }} />
            {folder.name}
  ...
  return (
    <div className="flex flex-col h-full border-r border-border/40 bg-bg-secondary/30 w-[160px] shrink-0">
      {/* Header */}
      <div className="px-3 pt-3 pb-1.5">
        <span className="text-[10px] font-semibold uppercase tracking-wider text-text-muted/60">Etiquetas</span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-1.5 py-1 space-y-0.5">
        ...
        >
          <svg width="12" height="12" viewBox="0 0 16 16" fill="none" className="shrink-0 opacity-70">
            <path d="M2 3.5A1.5 1.5 0 013.5 2h2.879a1.5 1.5 0 011.06.44l.622.621a1.5 1.5 0 001.06.439H12.5A1.5 1.5 0 0114 5v7.5a1.5 1.5 0 01-1.5 1.5h-9A1.5 1.5 0 012 12.5v-9z" stroke="currentColor" strokeWidth="1.3"/>
          </svg>
          <span className="flex-1 truncate">Todas</span>
          <span className={`text-[10px] tabular-nums ${activeFolder === null ? 'text-white/60' : 'text-text-muted/50'}`}>{totalItems}</span>
        </button>

        {/* Divider */}
        {folders.length > 0 && <div className="mx-2 my-1 border-t border-border/30" />}

        {/* Tags */}
        {folders.map(folder => (
          editingId === folder.id ? (
            <div key={folder.id} className="px-1.5 py-1.5 space-y-1.5">
              <input
                ref={editInputRef}
                value={editName}
                ...
                className="text-[11px] px-2 py-1 rounded-md bg-bg-primary border border-border/60 w-full text-text-primary focus:outline-none focus:border-accent/60"
              />
            ...
            >
              ...
              <span className={`text-[10px] tabular-nums ${activeFolder === folder.id ? 'text-white/60' : 'opacity-50'}`}>
      ...
      {/* Create tag button */}
      {!readOnly && (
        <div className="px-1.5 pb-2 pt-1 border-t border-border/30">
          {creating ? (
            <div className="px-1.5 py-1.5 space-y-1.5">
              <input
                ...
                placeholder="Nombre..."
                className="text-[11px] px-2 py-1 rounded-md bg-bg-primary border border-border/60 w-full text-text-primary focus:outline-none focus:border-accent/60 placeholder:text-text-muted/50"
              />
              <div className="flex gap-1 flex-wrap">
                {TAG_COLORS.map(c => (
                  <button
              ...
              <div className="flex gap-1">
                <button onClick={handleCreate} className="flex-1 text-[11px] px-1.5 py-1 rounded bg-accent/20 text-accent hover:bg-accent/30 transition-colors">
                  Crear
                </button>
                <button onClick={() => setCreating(false)} className="text-[11px] px-1.5 py-1 rounded text-text-muted hover:text-text-primary hover:bg-bg-hover transition-colors">
                  &times;
                </button>
              </div>
            </div>
            ...
            <button
              onClick={() => {
                setNewColor(TAG_COLORS[folders.length % TAG_COLORS.length]);
                setCreating(true);
              }}
              className="w-full text-[11px] px-2.5 py-1.5 rounded-md flex items-center gap-2 text-text-muted hover:text-accent hover:bg-accent/10 transition-all duration-150"
            >
...
// Backward compat for PublicSessionView
export { TagSidebar as FolderChips };
